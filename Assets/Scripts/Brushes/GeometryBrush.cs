...
namespace TiltBrush
{
    ...
    public abstract class GeometryBrush : BaseBrushScript
    {
        ...
        protected void SetVert(int iVert, int vp, Vector3 v, Vector3 n, Color32 c, float alpha)
        {
            ...
            // See if clamping value prevents errors
            int i = Mathf.Clamp(iVert + vp * NS, 0, m_geometry.m_Vertices.Count - 1);
        ...
        {
            // See if clamping value prevents errors
            int i = Mathf.Clamp(iVert + vp * NS, 0, m_geometry.m_Texcoord0.v2.Count - 1);
        ...
        {
            // See if clamping value prevents errors
            int i = Mathf.Clamp(iVert + vp * NS, 0, m_geometry.m_Texcoord0.v4.Count - 1);
        ...
        {
            // See if clamping value prevents errors
            int i = Mathf.Clamp(iVert + vp * NS, 0, m_geometry.m_Texcoord1.v3.Count - 1);
        ...
        {
            // See if clamping value prevents errors
            int i = Mathf.Clamp(iVert + vp * NS, 0, m_geometry.m_Texcoord1.v4.Count - 1);
